...
class FollowingViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        DataService.dataService.CURRENT_USER_FOLLOWING_REF.observe(FIRDataEventType.value, with: { snapshot in
            
            // The snapshot is a current look at our users data.
            self.users = []
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "FollowingViewCell") as? FollowingViewCell {
            cell.configureCell(userId: userId)
            return cell
        } else {
            return FollowingViewCell()
        }
    }
}
